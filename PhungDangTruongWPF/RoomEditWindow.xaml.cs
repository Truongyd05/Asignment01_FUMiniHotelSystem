...
namespace PhungDangTruongWPF
{
    public partial class RoomEditWindow : Window
    {
        ...
        private bool ValidateInput()
        {
            ...
            {
                MessageBox.Show("Sức chứa phải là số nguyên.", "Lỗi", MessageBoxButton.OK, MessageBoxImage.Warning);
                return false;
            }

            if (!string.IsNullOrWhiteSpace(txtPricePerDay.Text) && !decimal.TryParse(txtPricePerDay.Text, out _))
            {
                MessageBox.Show("Giá phòng phải là số.", "Lỗi", MessageBoxButton.OK, MessageBoxImage.Warning);
                return false;
            }
